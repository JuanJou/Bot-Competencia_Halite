...
while True:
    ...
    for ship in team_ships:
        ...
        planetas_factibles=[ordDict[tiempo] for tiempo in ordDict if es_mio(ordDict[tiempo],team_ships) and not ordDict[tiempo].is_full()]
        ...
        if full_attack_mode==False:
            if len(planetas_factibles)>0 and ship.calculate_distance_between(closest_enemy_ships[0])>10:
                target_planet=planetas_factibles[0]
                if ship.can_dock(target_planet):
                    command_queue.append(ship.dock(target_planet))
                    planetas_naves.append((target_planet,ship))
                else:
