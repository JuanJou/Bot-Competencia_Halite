...
for x in range(n):
    matriz_de_planetas.append([False]*m)

planetas=game.game_map.all_planets()

for p in planetas:
    x=p.x/16
    y=p.y/16
    matriz_de_planetas[x][y]=True'''
...
def funcion_relevancia(nave,planeta):
    ...
    factor_y=1/abs(mitad_alto-planeta.y)
    factor_x=1/abs(mitad_ancho-planeta.x)
    #DARLE MAS PESO A LOS PLANETAS QUE ESTAN EN EL CENTRO, TACTICAL ADVANTAGE

    tiempo=distancia//7+12//numero_naves_puerto+factor_y*10+factor_x*10
...
while True:
    ...
    for ship in team_ships:
        ...
        naves_cercanas_amigas=[cercanos[distance][0] for distance in cercanos if isinstance(cercanos[distance][0], hlt.entity.Ship) and distance<7 and cercanos[distance][0] in team_ships]

        naves_cercanas_enemigas=[cercanos[distance][0] for distance in cercanos if isinstance(cercanos[distance][0], hlt.entity.Ship) and distance<7 and cercanos[distance][0] not in team_ships]




        if len(naves_cercanas_enemigas)>len(naves_cercanas_amigas)+1:
            #RUNAWAY
            angulo = -ship.calculate_angle_between(naves_cercanas_enemigas[0])
            command_queue.append(ship.thrust(hlt.constants.MAX_SPEED,angulo))
        else:
